...
async function Quiz() {
  ...
  let totalQuestions = Number(
    (await inquirer.prompt({ name: "TotalQuestions", type: "number" }))
      .TotalQuestions
  );
  for (let i = 0; i < totalQuestions; i++) {
    let tempQuestion = await inquirer.prompt(quizInput).then((answer) => {
      return answer;
    });
  ...
  for (let i = 0; i < questionList.length; i++) {
    ...
    if (
      ...
      JSON.stringify(questionList[i].correct)
    ) {
      studentMarks++;
    }
    console.clear();
  ...
  console.log(
    `You gained ${studentMarks}/${questionList.length}, ${
      (studentMarks / questionList.length) * 100
    }%\n`
  );
}
Quiz();
